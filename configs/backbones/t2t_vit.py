...
class TokenAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (q * self.scale) @ k.permute([0, 1, 3, 2])
        attn = F.softmax(attn, -1)
        attn = self.attn_drop(attn)

        x = (attn @ v).permute([0, 2, 1, 3]).reshape([B, N, self.in_dim])
        x = self.proj(x)
        x = self.proj_drop(x)
...
class T2TViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=2, embed_dim=768, depth=12,
